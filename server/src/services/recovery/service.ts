...
export function recoveryService(db: Db, deps: { enqueueWakeup: RecoveryWakeup }) {
  ...
  async function reconcileStrandedAssignedIssues() {
    ...
    for (const issue of candidates) {
      ...
      // SHA-1866: count *all* recent issue_continuation_needed retries since the
      // assignee last commented, not just the latest run's marker. Unbounded
      // retries across reassignment cycles get capped at MAX_CONTINUATION_RETRIES
      // and force a `blocked` escalation for human review.
